...
| --- | --- | --- | --- |
| [HTTP/2 RST_STREAM](https://www.rfc-editor.org/rfc/rfc9113.html#name-rst_stream) and [HTTP/3 stream aborts](https://httpwg.org/specs/rfc9114.html) | Provide transport/message-stream cancellation and abort mechanisms. | They stop or abort HTTP streams, not necessarily server-side work that has already been accepted and outlives the request stream. | Introduction must state why transport cancellation is insufficient for durable operation resources. |
