...
| [RFC 9652 Link-Template](https://www.rfc-editor.org/rfc/rfc9652.html) | Defines the `Link-Template` HTTP response header for advertising templated links. | Can advertise templated cancel targets such as `/jobs/{id}/cancel`, but does not define cancellation behavior. | Strong published HTTPAPI WG hook for discoverable cancel affordance URIs. |
| [RFC 9264 Linkset](https://www.rfc-editor.org/rfc/rfc9264.html) | Defines linkset representations and a `linkset` relation type. | Can publish grouped links and affordances outside a single representation, but does not define cancellation semantics. | Possible carrier for operation affordance sets. |
| [Idempotency-Key Internet-Draft](https://datatracker.ietf.org/doc/draft-ietf-httpapi-idempotency-key-header/) | Defines an `Idempotency-Key` request header for fault-tolerant non-idempotent requests. | Useful for retry interaction, but does not define cancellation lifecycle semantics. Datatracker lists it as an expired Internet-Draft while still recording HTTPAPI WG Document state. | Reference as expired WG prior art for retries; do not describe it as an active dependency without rechecking status. |
| [HTTP Link Hints Internet-Draft](https://datatracker.ietf.org/doc/draft-ietf-httpapi-link-hint/) | Describes metadata hints for links. | A natural fit for advertising cancelability constraints, but Datatracker lists it as an expired Internet-Draft while still recording HTTPAPI WG Document state. | Useful expired WG design prior art if cancelability needs method/status/auth hints on a link. |
...
| [Events Query Internet-Draft](https://datatracker.ietf.org/doc/draft-gupta-httpapi-events-query/) | Defines an HTTP `QUERY`-based way to receive a current representation and subsequent event notifications. | Relevant to terminal-state observation without polling, not to cancellation request semantics itself. | Possible adjacent prior art for observing cancellation completion. |
...
5. Put route-level and manual TP/FP validation before further score-model refinement. If the
   ...
   root.
6. Motivate the work around generic and autonomous clients: the reusable value
   is letting a client discover and invoke cancellation without service-specific
   route knowledge.
